...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    dw = (np.transpose(x.reshape(N,D))).dot(dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = ( mean, var, newx, gamma, beta, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    mean, var, newx, gamma, beta, eps = cache
    x = newx*(np.sqrt(var)+eps) + mean
    ...
    dgamma = np.sum(dout*newx, axis=0)
    dnewx = dout * gamma

   
    dxmu1 = dnewx * 1./(np.sqrt(var) + eps)

    
    dxmu2 = 2 * (x-mean) * 1. /N * np.ones((N,D)) * (0.5 * (1. /np.sqrt(var+eps))* (-1. /((np.sqrt(var) + eps)**2) * np.sum(dnewx*(x-mean), axis=0)))
        ...
        for z in range(F):
            for j in range(Hprime):
                for k in range(Wprime):
                    out[i,z,j,k] = np.sum(x[i,:,j*stride:(j*stride+HH),k*stride:(k*stride+WW)]*w[z,:,:,:])+b[z] # The wx+b step taking into consideration the matching movement of filter and the input.
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
